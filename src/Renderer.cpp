...
void Renderer::DrawLine(const Vec3 &p0, const Vec3 &p1, const Vec4 &color)
{
    ...
    for (;;)
    { 
        DrawPixel(800, 600, x0, y0,p0.z, {color.x, color.y, color.z, color.w},y0);
        e2 = err;
        x2 = x0;
        if (2 * e2 >= -dx)
        { 
            if (roundf(x0) == roundf(x1))
                break;
            if (e2 + dy < ed)
                DrawPixel(800, 600, x0, y0 + sy,p0.z,{color.x, color.y, color.z, color.w},y0);
            err -= dy;
            x0 += sx;
        }
        if (2 * e2 <= dy)
        { 
            if (roundf(y0) == roundf(y1))
                break;
            if (dx - e2 < ed)
                DrawPixel(800, 600, x2 + sx, y0,p0.z,{color.x, color.y, color.z, color.w},y0);
            err += dx;
            y0 += sy;
        }
    }
